...
public class SetDaoAttributeController extends BaseController {
	...
	@FXML
	private CheckBox chkInsertBatch;// 创建更多
	@FXML
	...
	public void init() {
		DaoConfig config;
		if (indexController.getDaoConfig() != null) {
			config = indexController.getDaoConfig();
		} else {
	...
	public void onSaveConfig(ActionEvent event) {
		DaoConfig config = getThisConfig();
		try {
			LOG.debug("执行保存配置文件...");
			ConfigUtil.saveDaoConfig(config, "default");
			LOG.debug("保存配置文件-->成功!");
	 ...
	 * 加载设置信息
	 */
	private void loadConfig(DaoConfig config) {
		if (config == null) {
			return;
		}
		...
		chkSelectByPage.setSelected(config.isSelectAllByPage());
		selectByPageEvent(config.isSelectAllByPage());
	 ...
	 * @return
	 */
	private DaoConfig getThisConfig() {

		boolean isGetCount = chkGetCount.isSelected();
		boolean isSelectAll = chkSelectAll.isSelected();
		boolean isSelectAllByPage = chkSelectByPage.isScaleShape();
		...
		DaoConfig result = new DaoConfig(isGetCount, isSelectAll, isSelectAllByPage, isSelectObj, isSelectId, isInsert,
				isUpdate, isDelete, isInsertBatch, isDelOldFile, funGetCount, funSelectAll, funSelectAllByPage,
				funSelectObj, funSelectId, funInsert, funUpdate, funDelete, funInsertBatch, bsGetCount, bsSelectAll,
				bsSelectAllByPage, bsSelectObj, bsSelectId, bsInsert, bsUpdate, bsDelete, bsInsertBatch);
		return result;
	}
	 ...
	 * @return
	 */
	private DaoConfig getConfig(String configName) {
		DaoConfig result;
		try {
			LOG.debug("执行获取配置文件...");
			result = Optional.ofNullable(ConfigUtil.getDaoConfig(configName)).map(conf -> conf).orElse(new DaoConfig());
	...
	public void onSuccess(ActionEvent event) {
		indexController.setDaoConfig(getThisConfig());
